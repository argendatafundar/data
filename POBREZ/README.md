## POBREZ


Fuente: [Indicadores Sociales de Argentina (CEDLAS)](https://www.cedlas.econo.unlp.edu.ar/wp/en/estadisticas/isa/).


---

### [ISA_nbi-multidimensional_it1.csv](https://github.com/argendatafundar/data/blob/main/POBREZ/ISA_nbi-multidimensional_it1.csv)

**Gráficos:**
- https://graficador.argendata.fund.ar/grafico/POBREZ/1
- https://graficador.argendata.fund.ar/grafico/POBREZ/2

| **Variable** | **Tipo de dato** | **Descripcion** |
|:-------------:|:-------------:|:-------------:|
| province | alfanumerico | Provincia |
| year | entero | Año |
| nbi_rate | real | Porcentajes de hogares con NBI |

### [ISA_nbi-multidimensional_it3.csv](https://github.com/argendatafundar/data/blob/main/POBREZ/ISA_nbi-multidimensional_it3.csv)

**Gráficos:**
- https://graficador.argendata.fund.ar/grafico/POBREZ/3
- https://graficador.argendata.fund.ar/grafico/POBREZ/4

| **Variable** | **Tipo de dato** | **Descripcion** |
|:-------------:|:-------------:|:-------------:|
| year | entero | Año |
| semester | alfanumerico | Semestre |
| region | alfanumerico | Región geográfica |
| k_value | real | Umbral de privación multidimensional |
| pov_rate | real | Tasa de pobreza multidimensional (% de personas) |

### [ISA_pobreza_monetaria_it1.csv](https://github.com/argendatafundar/data/blob/main/POBREZ/ISA_pobreza_monetaria_it1.csv)

**Gráficos:**
- https://graficador.argendata.fund.ar/grafico/POBREZ/5

| **Variable** | **Tipo de dato** | **Descripcion** |
|:-------------:|:-------------:|:-------------:|
| year | entero | Año |
| pov_type | alfanumerico | Autor de la estimación de pobreza |
| poverty_rate | real | Tasa de pobreza en el GBA, 1974-1989 (% de hogares) |

### [ISA_pobreza_monetaria_it2.csv](https://github.com/argendatafundar/data/blob/main/POBREZ/ISA_pobreza_monetaria_it2.csv)

**Gráficos:**
- https://graficador.argendata.fund.ar/grafico/POBREZ/6

| **Variable** | **Tipo de dato** | **Descripcion** |
|:-------------:|:-------------:|:-------------:|
| year | entero | Año |
| period_type | alfanumerico | Tipo de período en el que se mide la pobreza |
| date | entero | Número de mes o semestre |
| survey | alfanumerico | Encuesta |
| region | alfanumerico | Región geográfica |
| poverty_rate | real | Tasa de pobreza en el GBA, 1988-2003 (% de personas) |

### [ISA_pobreza_monetaria_it3.csv](https://github.com/argendatafundar/data/blob/main/POBREZ/ISA_pobreza_monetaria_it3.csv)

**Gráficos:**
- https://graficador.argendata.fund.ar/grafico/POBREZ/23

| **Variable** | **Tipo de dato** | **Descripcion** |
|:-------------:|:-------------:|:-------------:|
| year | entero | Año |
| period_type | alfanumerico | Tipo de período en el que se mide la pobreza |
| date | entero | Número de mes o semestre |
| poverty_line | alfanumerico | Línea de pobreza nacional (indigencia o pobreza) |
| poverty_rate | real | Tasa de indigencia/pobreza (% de personas) |

### [ISA_global-poverty_it1.csv](https://github.com/argendatafundar/data/blob/main/POBREZ/ISA_global-poverty_it1.csv)

**Gráficos:**
- https://graficador.argendata.fund.ar/grafico/POBREZ/8

| **Variable** | **Tipo de dato** | **Descripcion** |
|:-------------:|:-------------:|:-------------:|
| geocodigoFundar | alfanumerico | Código geográfico Fundar |
| geonombreFundar | alfanumerico | Nombre geográfico Fundar |
| anio | entero | Año |
| poverty_line | real | Línea de pobreza internacional en USD diarios a PPA 2017 |
| poverty_rate | real | Tasa de pobreza (% de personas) |

### [ISA_global-poverty_it2.csv](https://github.com/argendatafundar/data/blob/main/POBREZ/ISA_global-poverty_it2.csv)

**Gráficos:**
- https://graficador.argendata.fund.ar/grafico/POBREZ/9

| **Variable** | **Tipo de dato** | **Descripcion** |
|:-------------:|:-------------:|:-------------:|
| geocodigoFundar | alfanumerico | Código geográfico Fundar |
| geonombreFundar | alfanumerico | Nombre geográfico Fundar |
| year | entero | Año |
| poverty_line | real | Línea de pobreza internacional en USD diarios a PPA 2017 |
| poverty_rate | real | Tasa de pobreza (% de personas) |

### [ISA_global-poverty_it3.csv](https://github.com/argendatafundar/data/blob/main/POBREZ/ISA_global-poverty_it3.csv)

**Gráficos:**
- https://graficador.argendata.fund.ar/grafico/POBREZ/10

| **Variable** | **Tipo de dato** | **Descripcion** |
|:-------------:|:-------------:|:-------------:|
| geocodigoFundar | alfanumerico | Código geográfico Fundar |
| geonombreFundar | alfanumerico | Nombre geográfico Fundar |
| year | entero | Año |
| poverty_line | real | Línea de pobreza internacional en USD diarios a PPA 2017 |
| poverty_rate | real | Tasa de pobreza (% de personas) |

### [ISA_region-pobreza_it1.csv](https://github.com/argendatafundar/data/blob/main/POBREZ/ISA_region-pobreza_it1.csv)

**Gráficos:**
- https://graficador.argendata.fund.ar/grafico/POBREZ/11
- https://graficador.argendata.fund.ar/grafico/POBREZ/12

| **Variable** | **Tipo de dato** | **Descripcion** |
|:-------------:|:-------------:|:-------------:|
| year | entero | Año |
| semester | alfanumerico | Semestre |
| region | alfanumerico | Región geográfica |
| poverty_line | alfanumerico | Línea de pobreza nacional (indigencia o pobreza) |
| poverty_rate | real | Tasa de indigencia/pobreza (% de personas) |

### [ISA_genero-pobreza_it1.csv](https://github.com/argendatafundar/data/blob/main/POBREZ/ISA_genero-pobreza_it1.csv)

**Gráficos:**
- https://graficador.argendata.fund.ar/grafico/POBREZ/13

| **Variable** | **Tipo de dato** | **Descripcion** |
|:-------------:|:-------------:|:-------------:|
| year | entero | Año |
| semester | alfanumerico | Semestre |
| gender | alfanumerico | Género |
| poverty_line | alfanumerico | Línea de pobreza nacional (indigencia o pobreza) |
| poverty_rate | real | Tasa de indigencia/pobreza (% de personas) |

### [ISA_genero-pobreza_it2.csv](https://github.com/argendatafundar/data/blob/main/POBREZ/ISA_genero-pobreza_it2.csv)

**Gráficos:**
- https://graficador.argendata.fund.ar/grafico/POBREZ/14

| **Variable** | **Tipo de dato** | **Descripcion** |
|:-------------:|:-------------:|:-------------:|
| year | entero | Año |
| semester | alfanumerico | Semestre |
| hh_gender | alfanumerico | Género del jefe de hogar |
| poverty_line | alfanumerico | Línea de pobreza nacional (indigencia o pobreza) |
| poverty_rate | real | Tasa de indigencia/pobreza (% de personas) |

### [ISA_laboral-pobreza_it1.csv](https://github.com/argendatafundar/data/blob/main/POBREZ/ISA_laboral-pobreza_it1.csv)

**Gráficos:**
- https://graficador.argendata.fund.ar/grafico/POBREZ/15
- https://graficador.argendata.fund.ar/grafico/POBREZ/16

| **Variable** | **Tipo de dato** | **Descripcion** |
|:-------------:|:-------------:|:-------------:|
| year | entero | Año |
| semester | alfanumerico | Semestre |
| labor_status | alfanumerico | Estatus laboral |
| poverty_line | alfanumerico | Línea de pobreza nacional (indigencia o pobreza) |
| poverty_rate | real | Tasa de indigencia/pobreza (% de personas) |

### [ISA_educacion-pobreza_it1.csv](https://github.com/argendatafundar/data/blob/main/POBREZ/ISA_educacion-pobreza_it1.csv)

**Gráficos:**
- https://graficador.argendata.fund.ar/grafico/POBREZ/17
- https://graficador.argendata.fund.ar/grafico/POBREZ/18

| **Variable** | **Tipo de dato** | **Descripcion** |
|:-------------:|:-------------:|:-------------:|
| year | entero | Año |
| semester | alfanumerico | Semestre |
| educ_level | alfanumerico | Nivel educativo |
| poverty_line | alfanumerico | Línea de pobreza nacional (indigencia o pobreza) |
| poverty_rate | real | Tasa de indigencia/pobreza (% de personas) |

### [ISA_etario-pobreza_it1.csv](https://github.com/argendatafundar/data/blob/main/POBREZ/ISA_etario-pobreza_it1.csv)

**Gráficos:**
- https://graficador.argendata.fund.ar/grafico/POBREZ/19

| **Variable** | **Tipo de dato** | **Descripcion** |
|:-------------:|:-------------:|:-------------:|
| year | entero | Año |
| semester | alfanumerico | Semestre |
| age_group | alfanumerico | Grupo etario |
| poverty_line | alfanumerico | Línea de pobreza nacional (indigencia o pobreza) |
| poverty_rate | real | Tasa de indigencia/pobreza (% de personas) |

### [ISA_etario-pobreza_it2.csv](https://github.com/argendatafundar/data/blob/main/POBREZ/ISA_etario-pobreza_it2.csv)

**Gráficos:**
- https://graficador.argendata.fund.ar/grafico/POBREZ/20

| **Variable** | **Tipo de dato** | **Descripcion** |
|:-------------:|:-------------:|:-------------:|
| year | entero | Año |
| semester | alfanumerico | Semestre |
| age_group | alfanumerico | Grupo etario/demográfico |
| poverty_line | alfanumerico | Línea de pobreza nacional (indigencia o pobreza) |
| poverty_rate | real | Tasa de indigencia/pobreza (% de personas) |

### [ISA_transf-pobreza_it1.csv](https://github.com/argendatafundar/data/blob/main/POBREZ/ISA_transf-pobreza_it1.csv)

**Gráficos:**
- https://graficador.argendata.fund.ar/grafico/POBREZ/21

| **Variable** | **Tipo de dato** | **Descripcion** |
|:-------------:|:-------------:|:-------------:|
| year | entero | Año |
| semester | alfanumerico | Semestre |
| poverty_line | alfanumerico | Línea de pobreza nacional (indigencia o pobreza) |
| pov_type | alfanumerico | Tipo de indicador (pobreza/indigencia con o sin transferencias, o diferencia) |
| poverty_rate | real | Tasa de indigencia/pobreza (% de personas) |

### [ISA_transf-pobreza_it2.csv](https://github.com/argendatafundar/data/blob/main/POBREZ/ISA_transf-pobreza_it2.csv)

**Gráficos:**
- https://graficador.argendata.fund.ar/grafico/POBREZ/22

| **Variable** | **Tipo de dato** | **Descripcion** |
|:-------------:|:-------------:|:-------------:|
| year | entero | Año |
| semester | alfanumerico | Semestre |
| poverty_line | alfanumerico | Línea de pobreza nacional (indigencia o pobreza) |
| pov_type | alfanumerico | Tipo de indicador (pobreza/indigencia con o sin transferencias, o diferencia) |
| poverty_rate | real | Tasa de indigencia/pobreza (% de personas) |
